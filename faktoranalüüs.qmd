...
Kõige lihtsam on selgitada sobiv faktorite arv ilmselt funktsiooniga `fa.parallel()`.
...
Soovitud pööramise saame faktorite arvutamisel täpsustada argumendiga `rotate`. Võrdleme, kuidas paiknevad faktorite kordajad erinevatel pööramistel kahe esimese faktori suhtes.

```{r}
faktorid <- fa(surm, nfactors = 7, rotate = 'varimax')
plot(faktorid$loadings[, 1:2])
...
Üldiselt kehtivad faktoranalüüsile samad eeldused, mis peakomponentide analüüsi puhul. Veelgi olulisem on aga veenduda selles, et tunnuste vaheline jagatud hajuvus on piisav selleks, et 

- andmepilv ei ole sfääriline (*sphericity*) ja 
- iga tunnus ei ole seotud ainult ühe faktoriga (*sampling adequacy*).
...
cortest.bartlett(korrel, n = nrow(surm))$p.value
# KMO mõõdik
KMO(surm)
```
...
Kordajad saame kätte faktori objekti osisest `loadings`. 
...
faktorid$loadings
```
Kordajate alusel saame muuhulgas anda faktoritele tähenduse. Nt esimene faktor võiks iseloomustada arsiabi kättesaadavust, kuna on seotud eelkõige nakkushaiguste ja lihtsamini ravitavate surmapõhjustega. Teine faktor mõjutab muuhulgas õnnetustega seotud surmapõhjusi, nii et võiks iseloomustada elukeskkonna ohtlikkust.
...
```{r}
head(faktorid$complexity)
```

Kahte faktorit saame korraga kujutada hajuvusjoonisel.
