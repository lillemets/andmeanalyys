...
- eeldame, et kogumis on väärtus mingi konkreetse teoreetilise jaotusega, mis võimaldab üksikute valimi omaduste alusel matemaatiliselt leida vahemikhinangu.


Siin vaatame eelkõige esimest, taasvalikut hõlmavat lähenemist, aga lõpus uurime lühidalt ka teoreetilse jaotuse abil usalduvahemiku arvutamist.

## Ühe tunnuse usaldusvahemikud
...
Veerus `salary` on aastane brutotöötasu eurodes ilma lisatasudeta. Teisendame selle meile tuttavaaks kuutöötasuks ja uurime, kuidas tasud jaotuvad.
...
Kuna usaldusvahemik oleneb sellest, kui kindlad me tahame olla, et tegelik väärtus seal asub, siis peame selle kindluse määrama. Enamasti leitakse usaldusvahemik kindusega 95%. Taasvalikut rakendadades tähendab see, et usaldusvahemiku määravad need piirid, kuhul jääb 95% kõikidest leitud valimite keskmistest. Seega alumine töötasu piir peab olema selline, millest väiksemaid väärtusi on 2,5% ja ülemisest piirist suuremaid väärtusi 2,5%. Sinna vahele jääb 95% kõikide valimite alusel leitud keskmistest töötasudest. Vastavate kvantiilide leidmiseks saame kasutada funktsiooni `quantile()`.
...
Need kaks töötasu väärtust `r quantile(keskmised, .025) %>% round` ja `r quantile(keskmised, .975) %>% round` ongi 95% usaldusvahemikud, mille vahele jääb suure tõenäosusega tegelik töötasu.

All oleval joonisel on leitud usaldusvahemikud kujutatud töötasu valimijaotuse tihedusfunktsioonil. Joontena esitatud usaldusvahemike vahele jääb 95% tihendusfunktsiooni kõvera alusest pindalast.
...
Eelnevate arvutuste üks mõte oli usaldusvahemike leidmise selgitamine. Tegelikult saab korduvvaliku alusel usaldusvahemikke leida ka lühemalt, kasutades funktsioone `boot()` ja `boot.ci()` vastavast laiendusest. Sellisel juhul tuleb kasutatav mõõdik vormistada veidi keerulisemalt, käsitsi loodava funktsioonina.
...
Näeme, et leitud töötasu usaldusvahemikud on sarnased varem käsitsi leitud usaldusvahemikele `r quantile(keskmised, .025) %>% round` ja `r quantile(keskmised, .975) %>% round`.

:::{.callout-warning}
Korduvvaliku käigus võetakse valimid juhuslikult. Seetõttu saame korduvvalikut korrates peaaegu alati veidi erineva valimi ja seega ka erinevad usaldusvahemikud. See erinevus on aga piisavalt väike, et hinnngut mitte märkimisväärselt muuta.
:::
...
Näeme, et naiste keskmine töötasu on tunduvalt madalam kui meestel, kuigi töötasude jaotused suurel määral kattuvad. Enne järelduste tegemist tuleb aga meeles pidada, et andmestikus on ainult valim ja tegelikult, kogumis ei pruugi üldse erinevust olla. Usutavama järeldus saame teha usaldusvahemike alusel. Leiame järgnevalt usaldusvahemikud naiste ja meeste töötasude kohta eraldi.

```{r}
# Naiste töötasu usaldusvahemikud
keskmisedN <- boot(tasu$salary[tasu$gender == "Female"], 
                   function(x,i) mean(x[i]), 
                   1000) 
boot.ci(keskmisedN, conf = 0.95, type = 'norm')
# Meeste töötasu usaldusvahemikud
...
:::{.callout-warning}
Usaldusvahemikel on palju tõlgendusviise. Õige on öelda, et usaldusvahemikud kehtivad 95% valimite korral ja tegemist on 95% usaldusvahemikega. Korduvvaliku alusel leitud usaldusvahemike korral ei ole aga ka täiesti vale öelda, et need kehtivad 95% tõenäosusega või et teglik väärtus jääb usaldusvahemikku 95% tõenäosusega.
:::

## Seose usaldusvahemikud

Korduvvalik ei võimalda usaldusvahemikke leida mitte ainult keskväärtuse, vaid mistahes parameetri jaoks. Alloleval joonisel paistab, et töökogemuse (`experience`) kasvades suureneb ka keskmiselt töötasu (`salary`). Meid võib huvitada, kas see seos kehtib ainult meie valimis või ka tegelikkuses. 
...
Arvutame usaldusvahemikud jällegi laienduse `boot` funktsioone kasutades. 

```{r}
korrelatsioonid <- boot(tasu[, c('salary', 'experience')], 
...
On selge, et 95% usaldusvahemikud ei kata nullpunkti. Sellest võime järeldada, et mitte ainult meie valimis, vaid ka üldiselt, kogumis on kogemuse ja töötasu vahel positiivne seos. Usaldusvahemike alusel võime järeldada, et korrelatsioonikordaja kogemuse ja töötasu vahel on kogumis 95% kindlusega vähemalt `r uv[1]`, aga mitte üle `r uv[1]`.

## Usaldusvahemikud statistika lähenemises

Eelnev usaldusvahemike leidmise viis valimeid simuleerides iseloomustab hästi andmeteadust. Enne arvuteid kujunenud statistika lähenemine usalduvahemike leidmisele põhineb rohkem matekaatika teoorial.
...
```{r}
tjaotus <- rt(1000, length(tasu$salary)) * sd(tasu$salary) + mean(tasu$salary)
par(mfrow = 1:2)
hist(tasu$salary, main = "Jaotus valimis", 20)
hist(tjaotus, main = "Eeldatav jaotus kogumis", 20)
```

Näeme, et meie küsitletute seas on jaotus veidi paremale kaldu, aga üldiselt siiski sarnane sellele vastavale t-jaotusele. Kui eeldame, et kogumis ongi väärtused jaotunud nii nagu t-jaotuse korral, siis saame usaldusvahemikud leida teoreetiliselt ilma suurel hulgal valimeid tekitamata.

Nimetatud eeldusel saab usaldusvahemikud leida matemaatiliselt kasutades valemit 
...
Need matemaatiliselt leitud usaldusvahemikud on väga sarnased varem korduvvaliku teel leitud usaldusvahemikele, milleks olid `r quantile(keskmised, .025) %>% round` ja  `r quantile(keskmised, .975) %>% round`.
